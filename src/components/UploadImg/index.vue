...
export default {
  ...
  methods: {
    ...
    scanUploadGet () {
      const token = this.scanToken.split('token=')[1]
    ...
    async submitUpload () {
      ...
      if (this.ruleForm.type == 0) {
        ...
        const urls = this.ruleForm.imgList.map((e) => {
          return e.url
        })
        if (urls.length) {
          if (this.loading) return
        ...
        const attId = this.ruleForm.imgList.map((e) => {
          return e.att_id
        })
    ...
    handleWebRemove (file) {
      const index = this.ruleForm.imgList.findIndex((e) => {
